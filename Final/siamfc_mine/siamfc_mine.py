...
from . import ops
from .dataloader import AllfocalLoader, AllframeLoader 
from .backbones import AlexNetV1
from .heads import SiamFC
from .losses import BalancedLoss
from .datasets import Pair
from .transforms import SiamFCTransforms
from .niqe import sharpfiles
...
class Net(nn.Module):
    ...
    def __init__(self, backbone, head):
        ...
        self.backbone = backbone #AlexNetV1
        self.head = head         
    
    def forward(self, z, x):
        z = self.backbone(z)    #1st Frame / Target
        x = self.backbone(x)    #Nst Frame / Input
...
class TrackerSiamFC(Tracker):
    ...
    def init(self, img, box):
        ...
        self.hann_window = np.outer(
            np.hanning(self.upscale_sz),                    #np.hanning() https://numpy.org/doc/stable/reference/generated/numpy.hanning.html
            np.hanning(self.upscale_sz))
        self.hann_window /= self.hann_window.sum()

        # search scale factors
        self.scale_factors = self.cfg.scale_step ** np.linspace(   
            -(self.cfg.scale_num // 8),                       
            self.cfg.scale_num // 8, self.cfg.scale_num)

        # search scale factors
        self.scale_factors_res = self.cfg.scale_step ** np.linspace(   
            -(self.cfg.scale_num_res // 2),                       
            self.cfg.scale_num_res // 2, self.cfg.scale_num_res)
    ...
    def update(self, count, img_files):  
        ...
        if count == 1:
            for fr, img_file in enumerate(img_files): 
                ...
                x.append(ops.crop_and_resize(img, self.center, self.x_sz*1.0, out_size=self.cfg.instance_sz, border_value=self.avg_color))  #0.7

        else : 
            for fr, img_file in enumerate(img_files): 
                img = ops.read_image(img_file)
                y = [ops.crop_and_resize(
                    img, self.center, self.x_sz * f,
                    out_size=self.cfg.instance_sz,
                    border_value=self.avg_color) for f in self.scale_factors_res]
                x.extend(y) 
        ...
        disp_in_image = disp_in_instance * self.x_sz * scale_factors[scale_id] / self.cfg.instance_sz
        self.center += disp_in_image 


        scale =  (1 - self.cfg.scale_lr) * 1.0 + self.cfg.scale_lr * scale_factors[scale_id]
        ...
        try : 
            for f, focal_dir in enumerate(focal_dirs): 
                ...
                if f == 0:
                    ...
                    sharp = sharpfiles(focal_dir) #focal_plane[27] #처음은 027.png
                    boxes[f, :], response, search, scale_id = self.update(f, sharp) #sharp
                else :
                    focal_planes = focal_plane[scale_id-1: scale_id + 1+1]
                    boxes[f, :], response, search, scale_id = self.update(f, focal_planes) #focal 이미지 리스트
                ...
                if visualize == True:            
                    ...
                    save_img, centerloc = ops.show_image(dimg, boxes[f, :], None, num_name=f)
                    save_img = cv2.putText(save_img, str(scale_id), (100, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                    savepath = "./image/{0:0=3d}.png".format(f)
                    cv2.imwrite(savepath, save_img)
                    #groundtruth
                    center_coo = "%d,%d,%d,%d\n"%(int(centerloc[0]), int(centerloc[1]), int(centerloc[2]), int(centerloc[3]))
                    gt.write(center_coo)
