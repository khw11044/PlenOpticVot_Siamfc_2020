...
#python test_groundtruth.py -vn ../data/NonVideo4_0
import os
import argparse
import cv2
import numpy as np
...
parser.add_argument('-vn', '--video_name', default='C:/Repos/vot/2020VOT_SiamFC/data/Video3', type=str) #C:\Repos\vot\2020VOT_SiamFC\vot_My_siamfc\image

args = parser.parse_args()
root = args.video_name

def get_frame1(video_name):
    images = dataLoader_img(video_name,'007') 
    for img in images:
        frame = cv2.imread(img)
        yield frame


f = open("recode/groundtruth3.txt", 'r') 
...
def main():
    ...
    for i, frame in enumerate(get_frame1(root)):
        ...
        frame = cv2.putText(frame, 'now_IOU : ' + str(area_acc), (200, 460), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
        frame = cv2.putText(frame, 'IOU : ' + str(area_acc_val), (200, 500), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
        frame = cv2.putText(frame, 'now_precision : ' + str(precision), (600, 460), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
        frame = cv2.putText(frame, 'precision : ' + str(prcision_value), (600, 500), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
