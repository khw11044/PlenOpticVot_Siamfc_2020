...
class TrackerSiamFC(Tracker):
    ...
    def init(self, img, box):
        ...
        self.scale_factors_init = self.cfg.scale_step ** np.linspace(   
            -(self.cfg.scale_num_init // 8),                       
            self.cfg.scale_num_init // 8, self.cfg.scale_num_init)
    ...
    def update(self, count, img_files): 
        ...
        if count == 1:
            ...
            for fr, img_file in enumerate(img_files):
                ...
                x.append(ops.crop_and_resize(img, self.center, self.x_sz*1.0, out_size=self.cfg.instance_sz, border_value=self.avg_color))
        else :
            ...
            for fr, img_file in enumerate(img_files): 
                ...
                responses = np.stack([cv2.resize(
                    ...
                    for u in responses])                            #responses.shape = 3x272x272
        ...
        try : 
            for f, focal_dir in enumerate(focal_dirs): 
                ...
                if f == 0:
                    ...
                    boxes[f, :], response, search, scale_id = self.update(f, sharp) #picked
                else :
