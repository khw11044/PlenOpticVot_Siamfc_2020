...
class TrackerSiamFC(Tracker):
    ...
    def init(self, img, box):
        ...
        self.scale_factors_init = self.cfg.scale_step ** np.linspace(   
            -(self.cfg.scale_num_init // 2),                       
            self.cfg.scale_num_init // 2, self.cfg.scale_num_init)
    ...
    def track(self,first_img, focal_dirs, show_imgs, box, lfs, visualize=False): #첫번째 박스지정하는 이미지:examplar, 두번째 위치가지정된 focal이미지
        ...
        try : 
            for f, focal_dir in enumerate(focal_dirs): 
                ...
                if f == 0:
                    ...
                    picked = focal_plane[lfs] #처음은 027.png
                    boxes[f, :], response, search, scale_id = self.update(f, picked) #picked
                else :
